...
begin
  ...
  if dmDados.fImprimirNota(wNotaID) then
    ...
    ShowMessage('Erro ao imprimir a nota.');
end;
...
begin
  ...
  try
    ...
    //dmDados.fCadastraNotas(wNotaJSON);
    if dmDados.fCadastraNotas(wNotaJSON) then
...
begin
  ...
  wResposta := dmDados.fBuscaNotaID(wNotaID);
...
begin
  try
      wProdutosJSONArray := dmDados.fBuscaProdutos;
